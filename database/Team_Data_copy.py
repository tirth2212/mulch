    ...
    try:
        ...
        for team_name, board_id in TRUCK_BOARDS.items():
            ...
            try:
                ...
                #    We'll build a dict of {title: groupId} for allowed titles only
                ...
                # Prepare a dict to store rows by group title
                grouped_data = {grp_title: [] for grp_title in allowed_groups.keys()}

                for item in all_items:
                    grp_title = item["group"]["title"]
                    if grp_title in allowed_groups:  # i.e. "Schedule" or "Production Review"
                        # parse columns
                ...
                # 6. Print tables for each allowed group
                for group_title, rows in grouped_data.items():
                    if rows:
                        print(f"\n📦 {team_name} - {group_title}:")
                        # Make sure all columns appear, even if some items are missing them
                        # We'll gather a set of all keys used across rows
                        all_keys = set()
                        for r in rows:
                            all_keys.update(r.keys())
            

                        # Convert dicts to list-of-lists for tabulate
                        # We'll keep a consistent column order based on TRUCK_COLUMN_MAPPING values
